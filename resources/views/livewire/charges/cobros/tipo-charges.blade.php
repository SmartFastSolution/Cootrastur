<div>
   @include('cruds.charges.charges.modal.modalcharges')
   <div class="card-body">
       <button type="button" class="btn btn-primary" data-toggle="modal" data-target="#createCobros"> Crear un Cobro</button>
   </div>
                 ...
                 @if ($data->isNotEmpty())
                     @foreach ($data as $p)
                         <tr>
                             ...
                             <td width="10px">
                                 <button class="btn btn-success" data-toggle="modal" data-target="#createCobros"
                                     wire:click.prevent="editCharges({{ $p->id }})">
                                     Editar
                                 </button>
                             </td>
                             <td width="10px">
                                 <button class="btn btn-danger"
                                     wire:click.prevent="$emit('eliminarRegistro','Seguro que deseas eliminar este Cobro?','eliminarCobros', {{ $p->id }})">
                                     Eliminar
                                 </button>
                             </td>
                         </tr>
