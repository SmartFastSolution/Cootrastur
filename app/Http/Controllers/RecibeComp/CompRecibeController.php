    ...
    public function AprobarFactura(Request $request){
        ...
        if($estado->status == "BORRADOR"){
            ...
            if($estado->status == "APROBADO"){
                ...
                foreach($detalle as $d) {
                    $valorPendiente = DB::table('partner_aditional_detail')->where('number_voucher',$d->voucher_deuda)->where('code_account',$d->code_account)->where('key_account',$d->key_account)->where('id_partner',$estado->id_proveedor)->where('status','PENDIENTE')->first();
                    if(isset($valorPendiente)){
                        $valorActualizar = number_format($valorPendiente->value_pending + $d->value_credito , 2, '.', "");
